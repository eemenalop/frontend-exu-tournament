...
export default function StatScopeFilter({ setSelectedStatScope }) {
    const [selected, setSelected] = useState(statScopes[0]);
    ...
    return (
        <Listbox value={selected} onChange={handleChange}>
            <div className="relative inline-block text-left justify-start mx-1">
                <ListboxButton className="relative w-32 cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-red-700 sm:text-sm sm:leading-6">
                    <span className="block truncate">{selected.scope}</span>
                ...
                >
                    {statScopes.map((scope) => (
                        ...
                        >
                            <span className="block truncate font-normal group-data-[selected]:font-semibold">{scope.scope}</span>
